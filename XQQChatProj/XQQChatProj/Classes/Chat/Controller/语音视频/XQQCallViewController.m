//
//  XQQCallViewController.m
//  XQQChatProj
//
//  Created by XQQ on 2016/11/30.
...
@interface XQQCallViewController (){
...
@implementation XQQCallViewController
...
#warning 要提前设置视频通话对方图像的显示区域
    ...
    if (_isIncoming) {
        //call.waiting
        _statusLabel.text = NSLocalizedString(@"等待接听...", @"Waiting to answer...");
        [_actionView addSubview:_answerButton];
        [_actionView addSubview:_rejectButton];
    }
    else{
        //call.connecting
        _statusLabel.text = NSLocalizedString(@"连接中...", @"Connecting...");
...
- (void)_initializeCamera
{
    ...
    _smallView = [[UIView alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 110, CGRectGetMaxY(_statusLabel.frame), width, height)];
    ...
    if(!error){
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"错误" message:error.localizedFailureReason delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alertView show];
    }
                           ...
                           error:(EMError *)error
{
    ...
    if(error){
        ...
        UIAlertView *errorAlert = [[UIAlertView alloc] initWithTitle:@"错误" message:error.description delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
...
- (void)rejectAction
{
    ...
    [self.view hideToast];
    //call.rejected
    [self.view makeToast:@"断开..." duration:1.f position:@"center"];
...
- (void)answerAction
{
    [self.view hideToast];
    //call.init
    [self.view makeToast:@"建立连接..." duration:1.f position:@"center"];
    [self _stopRing];
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
...
- (void)hangupAction
{
    ...
    [self _stopRing];
    [self.view hideToast];
    [self.view makeToast:@"挂断..." duration:1.f position:@"center"];
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
...
+ (BOOL)canVideo
{
    if([[[UIDevice currentDevice] systemVersion] compare:@"7.0"] != NSOrderedAscending){
        if(!([AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo] == AVAuthorizationStatusAuthorized)){
            UIAlertView * alt = [[UIAlertView alloc] initWithTitle:@"没有摄像头权限" message:@"请打开-设置-隐私-相机" delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
            [alt show];
            return NO;
        }
    }
